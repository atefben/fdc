...
class InfoAdmin extends Admin
{
    ...
    public function filterCallbackJoinTwiceTranslations($queryBuilder, $alias, $status)
    {
        ...
        if (!$joined) {
            $queryBuilder
                ->leftjoin('Base\CoreBundle\Entity\InfoArticle', 'na1', 'WITH', "na1.id = {$alias}.id")
                ->leftjoin('Base\CoreBundle\Entity\InfoAudio', 'na2', 'WITH', "na2.id = {$alias}.id")
                ->leftjoin('Base\CoreBundle\Entity\InfoImage', 'na3', 'WITH', "na3.id = {$alias}.id")
                ->leftjoin('Base\CoreBundle\Entity\InfoVideo', 'na4', 'WITH', "na4.id = {$alias}.id")
