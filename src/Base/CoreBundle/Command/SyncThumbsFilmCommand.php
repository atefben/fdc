 ...
 */
class SyncThumbsFilmCommand extends BaseCommand
{
    ...
    {
        $this->setName('base:media:sync-thumbnails-film')
            ->setDescription('Sync uploaded image thumbs with new media formats')
            ->setDefinition(array(
                    new InputArgument('festivalYear', InputOption::VALUE_REQUIRED, 'The movie festival year', 71),
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $movies = $em->getRepository('BaseCoreBundle:FilmFilm')->findBy(array('festival' => $festivalYear));

        $output->writeln('Movies count:'. count($movies));
