 ...
 */
class CcmAWSCallbackCommand extends ContainerAwareCommand
{
    /**
     * configure function.
     *
    ...
    {
        $this
            ->setName('ccm:aws:callback')
            ->setDescription('Update job status for CcmMediaVideoTranslation and CcmMediaAudioTransation')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $medias = $em->getRepository('FDCCourtMetrageBundle:CcmMediaVideoTranslation')->findBy(array('jobMp4State' => '1'));
        $this->batch($input, $output, $medias, 'mp4');

        $medias = $em->getRepository('FDCCourtMetrageBundle:CcmMediaVideoTranslation')->findBy(array('jobWebmState' => '1'));
        $this->batch($input, $output, $medias, 'webm');

        $medias = $em->getRepository('FDCCourtMetrageBundle:CcmMediaAudioTranslation')->findBy(array('jobMp3State' => '1'));
