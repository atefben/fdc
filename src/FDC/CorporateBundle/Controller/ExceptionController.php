...
class ExceptionController extends \Symfony\Bundle\TwigBundle\Controller\ExceptionController
{
    ...
    protected $corporateDomain;
    /**
     * @var ExceptionManager
     */
    protected $corpoExceptionManager;

    public function __construct(\Twig_Environment $twig, $debug, NotFoundExceptionManager $notFound404Manager, HeaderFooterManager $headerFooterManager, $corporateDomain)
    {
    ...
    public function showAction(Request $request, FlattenException $exception, DebugLoggerInterface $logger = null)
    {
        ...
        if($requestHeaders['HOST'] == $this->corporateDomain)
        {
        ...
        } elseif ($requestHeaders['HOST'] == $this->corporateDomain && $this->debug == false) {
            $locale = $request->getLocale();

            if ($exception->getStatusCode() === 404) {

                return $this->corpoExceptionManager->render404Page($locale);
            }
            
            if ($exception->getStatusCode() === 500) {
                
                return $this->corpoExceptionManager->render500Page($locale);
            }
        }
    ...
    /**
     * @param string $corpoDomain
     */
    public function setCcmDomain($corpoDomain)
    {
        $this->corpoDomain = $corpoDomain;
    }

    /**
     * @param ExceptionManager $corpoExceptionManager
     */
    public function setCorpoExceptionManager($corpoExceptionManager)
    {
        $this->corpoExceptionManager = $corpoExceptionManager;
    }
}
